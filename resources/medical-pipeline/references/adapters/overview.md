...
**Status**: Stable as of MSRA v3.6.4
...
MSRA provides three Python reference adapters in [`scripts/adapters/`](../../../scripts/adapters/). Users are expected to write their own adapters for non-reference sources. The three reference adapters are starting points, not production tools.

## 2. Why a contract, not a plugin API

MSRA deliberately does NOT run adapter code itself. The adapter runs in the user's environment, reads the user's data, and emits YAML files. MSRA reads those YAML files. Rationale:

- Each corpus source (Zotero, Obsidian, custom) has its own versioning, auth, and data quirks. Pinning one plugin API would force MSRA to track all of them.
- Keeping the boundary at "YAML files on disk" means a user can use any language, any scheduling mechanism, and any privacy posture without touching MSRA internals.
- MSRA remains a writing/review-layer tool; corpus integration stays in user-owned code.
...
| `contamination_signals` | object | v3.7.3 + v3.9.0 + v3.11 contaminated-source advisory field (spec v3.7.3 搂3.2 + v3.9.0 搂3.4鈥撀?.5 + v3.11 #182 Delta 1). |
...
| `venue` | string | 鈥?|
| `venue_type` | string | v3.10 (spec 搂3 PR-B item 2). |
| `venue_type_provenance` | string | v3.10 (spec 搂3 PR-B item 3). |
| `venue_type_source` | string | v3.10 (spec 搂3 PR-B item 4). |
...
Publishers typically retain rights to abstracts; user notes often quote copyrighted material. The schema marks these fields as PRIVATE and does NOT enforce anything at the CI level. **If you publish a passport (e.g., commit it to a public repo or share it on the web), you are responsible for removing or clearing these fields.** MSRA consumers treat both fields as optional; omitting them never causes failure.
...
- **Zotero Web API**: one that calls `api.zotero.org` with a user API token. Not shipped with MSRA by design (auth and rate-limit management would pull MSRA into data-layer territory).
...
## 10. Relationship to other MSRA artifacts
...
- [`medical-pipeline/references/passport_as_reset_boundary.md`](../passport_as_reset_boundary.md): `literature_corpus[]` is consumed across reset boundaries like any other passport field.
- MSRA agents that consume `literature_corpus[]` are **deferred** to v3.6.5+. As of v3.6.4, the field is a defined input port with no runtime consumer; adapters produce it, future MSRA versions read it.
