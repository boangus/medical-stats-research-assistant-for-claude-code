...
1. Call `validate_gold_set(tuples)`. Any of the four spec 搂7.7 rules (a)/(b)/(c)/(d) failing raises `GoldSetValidationError` with the offending tuple index and the rule name. Do NOT catch the exception 锟?fix the gold set.
...
- Alignment tuples (all four `expected_judgment` values including `RETRIEVAL_FAILED`) 锟?`retrieved_excerpt = tuple.ref_text_excerpt` (may be `null` for `RETRIEVAL_FAILED` tuples), `active_constraints=[]`. The runner does NOT pre-filter `RETRIEVAL_FAILED` tuples 锟?the judge stub is responsible for returning the matching label.
- Constraint tuples 锟?`retrieved_excerpt = tuple.ref_text_excerpt` (typically `null`), `active_constraints=[{"constraint_id": ..., "rule": ..., "scope": "MNC" | "NC"}]`. Scope is derived from `constraint_under_test_id`: `MNC-N` 锟?`"MNC"`, `NC-CN-M` 锟?`"NC"`. Malformed ids are rejected by `_derive_constraint_scope` with a diagnostic naming the expected shape.
...
- **Aggregate** (the rates that T-C1 gates against): a tuple contributes a FN when `expected_judgment 锟?actual judgment`. For alignment, every mismatched tuple counts as both FN (for the expected class) and FP (for the wrongly-picked class), so aggregate FNR and FPR are symmetric. For constraint tuples, VIOLATED is the positive class (gate-refuse signal); the matrix is the standard binary form.
...
- A regression (judge quality degraded 锟?same gold set, higher FNR/FPR than the previous run).
- A spec bump (tighter thresholds 锟?same judge, but T-C1 now demands more).
...
- Detect rubric-discrimination-power problems on the LLM-as-judge side 锟?that's the `#89 / gold fixtures` work tracked separately (per spec 搂2 out-of-scope).
- Replace the post-calibration ramp-on plan recorded in `medical-pipeline/SKILL.md` mode flags. v3.8.0 ships the audit agent dispatch as opt-in default-OFF; T-C1 passing on the canonical gold set is necessary but not sufficient evidence for default-ON.

If a deploying operator brings a gold set that is itself biased (all tuples from one venue, all post-2024, all heavily-redacted), calibration reports a biased profile. Emit a warning during intake if the gold set looks clustered 锟?this protocol does not currently auto-detect clustering, but the operator's pre-deployment review should.

**Partial-evidence regression IS caught (#213).** One failure the aggregate gate alone would NOT catch: a judge that regresses to emitting bare `UNSUPPORTED` on compound claims instead of decomposing them. Because partial fixtures carry `expected_judgment=UNSUPPORTED`, the aggregate FNR stays green on that regression. The `partial_support` subset metric exists precisely to surface it 锟?`miss_rate > 0` means the judge stopped producing inspectable `sub_claim_breakdown` decompositions even though it got the headline label right. Treat a non-zero `partial_support.miss_rate` as a judge-quality regression on the 搂F.3.2 partial-evidence trap, distinct from an aggregate FNR/FPR threshold breach.
...
|---|---|
| Stage 4 锟?5 audit dispatch | Calibration runs offline; results inform the `ARS_CLAIM_AUDIT` ramp-on decision. |
| Cite-Time Provenance Finalizer | No direct coupling 锟?calibration evaluates the judge in isolation; the finalizer's matrix is downstream. |
| `/MSRA-mark-read` clearance | Calibration does not interact with mark-read state; HIGH-WARN-CLAIM-NOT-SUPPORTED stays non-clearable regardless of calibration outcome. |
...
- **Threshold values**: FNR < 0.15 + FPR < 0.10. Tightened from the reviewer-mode 0.17 / 0.50 Lu 2026 reference points because the audit unit (per-claim) is simpler than the reviewer unit (whole paper) 锟?the gate should track the stricter end of plausible LLM-as-judge accuracy.
...
- **Cross-model verification**: out of scope for the calibration runner 锟?`ARS_CROSS_MODEL` interacts with the audit agent dispatch path, not the calibration script. An operator wanting cross-model calibration runs the runner twice with different `judge_model` settings.
...
- Acceptance criterion: FNR < 0.15 + FPR < 0.10 gates (internal spec).
- Parent agent: `medical-pipeline/agents/claim_ref_alignment_audit_agent.md` 锟?dispatches calibration mode and consumes the report's thresholds block. The agent prompt and this doc form a two-way pair following the v3.6.5 protocol-doc convention (`literature_corpus_consumers.md` 锟?`bibliography_agent.md`).
- Reviewer calibration baseline: `peer-review/references/calibration_mode_protocol.md` (same vocabulary, different unit of analysis).
- Lu, C. et al. (2026). *Nature* 651, 914-919 锟?Table 1 reference rates for LLM-vs-human agreement on whole-paper decisions; this protocol uses tighter thresholds for the simpler per-claim unit.
