...
The user has a completed paper titled "The Impact of Micro-Credential Certification on Employability of Vocational Education Students in Taiwan," which has received peer review comments from a journal (3 major + 4 minor). The user employs medical-paper's revision mode for systematic revision. This example demonstrates the complete workflow from comment parsing, citation correction, review verification, to the Response to Reviewers letter.
...
**m1.** The third sentence of the Chinese abstract 鈥?"This study found that micro-credential certification has a significant positive impact on student employability" 鈥?is too vague and does not mention the magnitude of the effect size.
...
## output_formatter Parses Revision Comments
...
The output_formatter recommends the following revision order:
...
## reference_checker Corrects Citation Issues
...
Found that the in-text citation on page 23 reads "Chen et al.锛?023锛?" but the reference list entry is "Chen, Y.-L., & Wang, S.-T.锛?023锛?" which has only two authors and should not use "et al."

Before correction:
```
Chen et al.锛?023锛塮ound that micro-credential certification helps...
```

After correction:
```
Chen and Wang锛?023锛塮ound that micro-credential certification helps...
```
...
In conjunction with M2's literature update requirement, the reference_checker verifies format compliance of newly added references:
...
## mock_reviewer Reviews the Revised Version
