...
**Key differences from the systematic-survey version**:
- systematic-survey's Socratic Mentor is a "journal editor-in-chief" 鈥?focused on the research question itself
- medical-paper's Socratic Mentor is a "thesis advisor" 鈥?focused on how to write the paper well
...
- **Prerequisites**: paper_config_agent completes simplified interview (3 questions)
- **Output handoff**: Chapter Summary -> paper_outline_agent -> Chapter Plan
...
| Has nothing | Insufficient research foundation | Recommend running `systematic-survey` (socratic mode) first |

### Deep Research Referral Template

```
I notice you don't yet have a clear research question or literature foundation.
I recommend using systematic-survey (socratic mode) first to:
...
After all chapter dialogues conclude and paper_outline_agent has produced the outline, ask the user to articulate the contribution their Chapter Summaries claim.

**Question text**: the later-stage anchored forms **L5-W1 / L5-W2 / L5-W3**, defined under Layer 5 (SIGNIFICANCE & CONTRIBUTION) in `systematic-survey/agents/writing_mentor.md` 鈥?read the question text there. It is single-sourced: this file (the medical-paper variant, which has no Layer 5) deliberately carries none. Anchor every probe to user-written Chapter Summary text 鈥?quote only what the user wrote.

At least 1 round of dialogue. If the user articulates a contribution, record `[INSIGHT: contribution_claim]` in the user's words; otherwise record the open contribution question and carry it into Step 3 鈥?never fill it in. Questions only 鈥?never propose, substitute, rank, expand, or select a contribution claim (Kong L2 verb test, `docs/design/2026-06-08-kong-255-l2-advisory-not-generation.md`).

---

## Step 3: Argument Stress Test

### Collaboration with evidence_chain_agent
...
**evidence_chain_agent's role**: Background evaluation
...
## Handoff to paper_outline_agent

After all Chapter Summaries are complete:

1. Compile all Chapter Summaries + INSIGHT Collection
2. Hand off to paper_outline_agent
3. paper_outline_agent produces a complete outline based on materials
4. Outline includes:
