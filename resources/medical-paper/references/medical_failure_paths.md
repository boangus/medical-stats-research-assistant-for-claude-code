...
This document records the failure scenarios that the medical-paper skill may encounter at each stage, their trigger conditions, and handling strategies. All agents should refer to this guide when they detect a failure scenario.
...
|---|---------|---------|--------|---------|
| F1 | Insufficient research foundation | Plan mode Step 0 finds no RQ / no data | High | Recommend running `systematic-survey` first |
...
| F9 | Incomplete handoff materials | From systematic-survey but missing key materials | Low | List missing items, suggest supplementing or re-running |
...
3. Recommend using systematic-survey (socratic mode)
4. Explain that they can come back to continue after systematic-survey is completed
...
I recommend you first use the systematic-survey tool to:
...
**Trigger Timing**: Phase 2 (paper_outline_agent)
...
**Trigger Timing**: Phase 4 (after manuscript_drafter completes)
...
**Trigger Timing**: Phase 5a (reference_checker)
...
**Trigger Timing**: Phase 5b (bilingual_abstract_agent)
...
**Trigger Timing**: Phase 6 (mock_reviewer issues Reject)
...
**Trigger Timing**: paper_config_agent detects systematic-survey materials but they are incomplete
...
3. Recommend:
   a. Return to systematic-survey to complete the missing parts
   b. Or supplement within medical-paper (add Phase 0 interview questions)
```
...
**How to Resume**: Bring this record and restart medical-paper; can continue from Phase {N}
```
...
```
F1 (Insufficient research foundation) 鈫?Recommend systematic-survey 鈫?May encounter F9 (incomplete materials) upon return
...
| **Formatting Non-Compliance** | Immediate rejection for template/length/style violations | Review target journal's author guidelines; use `output_formatter` to reformat; resubmit (often same journal accepts after formatting fix) |
| **Poor Opening** | No specific reason given; likely the abstract/introduction failed to hook | Rewrite abstract with the CARS model (Create A Research Space); lead with the gap, not the background; have `mock_reviewer` evaluate the new opening |
...
| F9 | paper_config_agent performs a complete materials check when detecting a handoff |
...
| F12 | paper_config_agent detects whether this is a conference paper expansion; calculate new content ratio early |
