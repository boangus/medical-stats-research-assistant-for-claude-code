...
RAISE's official scope is **evidence synthesis** (systematic review, meta-analysis, scoping review, rapid review, evidence mapping). When MSRA applies RAISE **principles** to non-evidence-synthesis work 鈥?e.g., `medical-paper full` on primary research 鈥?this is a **principle extension**, not official RAISE compliance. The compliance checker will only claim RAISE compliance for outputs produced in `systematic_review` or `other_evidence_synthesis` modes.

**Used by**: pipeline skill (Stage 2.5 / 4.5).

> **Attribution**: 本文件基于 RAISE 框架咨询稿（NIHR ESG, 2025年7月）整理，用于学术研究和证据合成合规检查。原始内容版权归 NIHR ESG Best Practice Working Group 所有。引用: Thomas J, et al. NIHR ESG 2025。如需最新版本，请访问 EPPI-Centre 网站。

## Four principles

Each principle carries a definition, MSRA check procedure, and pass/warn/fail criteria. These are the universal kernel of RAISE and are checked in every mode.
...
**MSRA check (Stage 2.5 focus):** At Stage 2.5, verify `methodology_blueprint` specifies reviewer count, qualifications, and adjudication mechanism. At Stage 4.5, verify manuscript describes the human oversight applied in practice.
...
**MSRA check (Stage 4.5 focus):** Verify manuscript lists every AI tool used, at what stage, with prompts/parameters/versions. Cross-reference against `user_metadata.ai_tools_used`.
...
**MSRA check:** Verify presence of `passport.repro_lock` (v3.3.5 feature) OR equivalent manuscript description (model version, seeds, prompt, data access details). Stochasticity must be declared per `artifact_reproducibility_pattern.md`.
...
**MSRA check (Stage 2.5 focus):** Verify manuscript describes why each AI tool was chosen for its specific task. Look for pilot-phase evidence OR prior validation citation.
...
Used when `raise.mode == "full"` (SR and other_evidence_synthesis). Each role carries 2鈥? responsibilities extracted from the RAISE consultation PDF. The agent maps MSRA users and MSRA itself onto the roles they occupy.

**Default role attribution:**
- MSRA user (the human running the skill): Evidence Synthesists
- MSRA itself (the skill suite): AI Development Teams + Methodologists (dual role)
...
**MSRA check:** manuscript contains author responsibility statement; AI-usage disclosure is present; ethics/IRB section addresses AI use.
...
**MSRA self-declaration:** MSRA is open-source (CC BY-NC 4.0); CHANGELOG tracks limitations; calibration results published per `calibration_mode_protocol.md`.
...
**MSRA self-declaration:** MSRA cross-model verification per `cross_model_verification.md` provides one form of independent validation; sixth-reviewer peer review remains planned.
...
**MSRA note:** Out of MSRA's direct control; surfaced in compliance_report as stakeholder context.
...
**MSRA note:** Out of direct scope; listed for ecosystem completeness.
