...
| 防御规则 | 报告精确 P 值（P=0.XX）和效应量+95%CI；用 `src/shared/reporting-guidelines/statcheck_patterns.py` 自动验证 P 值与统计量一致性 |
...
> 基于 resources/statistics-methods/statistical_constraints.md 中定义的约束规则。
...
| 防御规则 | 严格执行 resources/chart-styles/publication_figure_standards.md：三行强制 rcParams；语义化调色板；仅左+下轴线；SVG 首要导出；变量名遵循 variable_naming_conventions.md |
...
| 防御规则 | 严格执行 resources/chart-styles/variable_naming_conventions.md：三列命名体系（代码变量名/规范显示名/英文显示名）；全流程一致性检查（V-R01~R07） |
