...
> 基于 Agent Team Orchestration 模式，为医学统计分析流水线设计多 Agent 协作架构。版本: 1.0.0 | 2026-06-22
...
```
┌──────────────────────────────────────────────────────────────────────────────┐
│                         MSRA Multi-Agent Framework                            │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │                        Core Infrastructure                               │  │
│  │  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ┌─────────────────┐  │  │
│  │  │  IAgent     │  │ MessageBus  │  │ TaskQueue  │  │ ConflictResolver│  │  │
│  │  │  Interface  │  │ (Async MQ)  │  │ (Priority) │  │ + Arbitrator    │  │  │
│  │  └─────────────┘  └─────────────┘  └─────────────┘  └─────────────────┘  │  │
│  │  ┌─────────────────────────────────────────────────────────────────────┐ │  │
...
│  │                          Agent Layer                                      │ │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────────┐  │ │
│  │  │ Data          │ │ Method        │ │ Exec Runner   │ │ Exec Inference│  │ │
│  │  │ Validator     │ │ Consultant    │ │              │ │              │  │ │
│  │  └───────────────┘ └───────────────┘ └───────────────┘ └───────────────┘  │ │
│  │  ┌───────────────┐ ┌───────────────┐ ┌───────────────┐ ┌───────────────┐  │ │
│  │  │ QC Inspector  │ │ Report Expert │ │ Paper Writer  │ │ Arbitrator    │  │ │
│  │  │              │ │              │ │              │ │ (Built-in)    │  │ │
│  │  └───────────────┘ └───────────────┘ └───────────────┘ └───────────────┘  │ │
│  └──────────────────────────────────────────────────────────────────────────┘ │
└──────────────────────────────────────────────────────────────────────────────┘
...
| 组件 | 文件 | 功能 |
|------|------|------|
| **IAgent Interface** | `agents/core/interfaces.py` | 标准Agent接口定义 |
...
| **ConflictResolver** | `agents/core/conflict_resolution.py` | 冲突检测与解决，支持仲裁 |
...
# 创建完整框架
message_bus, task_queue, cache, conflict_resolver = create_agent_framework()

# 使用缓存
result = await cache.get("key", compute_if_missing=lambda: compute_value())
```
...
│ 清洗    │ │ SAP 制定   │ │ 自愈Debug │ │ 质量检查    │ │ 一致性检查   │
│ 验证    │ │ 计划审查   │ │ Phase 0-6 │ │ Phase 7-9    │ │ 偏差检测     │
└─────────┘ └───────────┘ └──────────┘ └──────────────┘ └──────────────┘
```
...
**专长**:
- SAP 质量门闸（Stage 2.5，8 项阻断检查）
- 结果质量门闸（Stage 3.5，9 项阻断检查）
- 数值一致性检查（跨表格/图表/文本）
- 异常结果标记与偏差评估
...
2. 变量定义明确 → 衍生变量构造逻辑可复现
3. 缺失机制评估 → 缺失模式与处理策略匹配
4. 盲态审核完成 → 质疑关闭状态
5. 数据库锁定 → 版本/时间/清单完整性
6. 逻辑一致性 → 交叉字段与日期顺序
7. 可重复性 → 清洗脚本独立复现
8. 隐私合规完成 → PHI/标识符处理报告
9. 输出 → 门闸报告（阻断条件通过/完全通过）
...
| **Stage 5.1: Literature** | **systematic-survey** | — (融合点 C: MSRA seed 扩展) |
| **Stage 5.2: Writing** | **medical-paper** | — (融合点 D/F: 方法学/表格图表复用) |
| **Stage 5.5: Integrity** | **integrity verification** | Pipeline (融合点 B: Stage 3.5 门闸复用) |
| **Stage 5.6: Review** | **medical-paper-reviewer** | — (5 人评审团) |
| **Stage 5.7: Revision** | **medical-paper** | — (修订 + Response to Reviewers) |
| **Stage 5.8: Final Integrity** | **integrity verification** | Pipeline |
| **Stage 5.9: Finalize** | **medical-paper** | — (MD + DOCX + PDF) |
...
Pipeline Orchestrator
  ...
  └── Stage 4 → 扮演 Report Expert 角色
                → QC Inspector 规范检查
                → 到 Stage 4 Checkpoint: [A] 结束 / [B] Paper Track
                │
                ├── [A] → Pipeline 结束 (report_only)
                │
                └── [B] → Stage 5.0: Pipeline 生成 Handoff Bundle
                         → dispatch → medical-pipeline
                         │
                         ├── Stage 5.1 → systematic-survey (文献检索)
                         ├── Stage 5.2 → medical-paper (论文写作)
                         ├── Stage 5.5 → integrity verification
                         ├── Stage 5.6 → medical-paper-reviewer (评审)
                         ├── Stage 5.7 → medical-paper (修订)
                         ├── Stage 5.8 → integrity verification
                         ├── Stage 5.9 → medical-paper (定稿)
                         └── Stage 6  → Process Summary
```

**核心原则**: Pipeline Orchestrator 始终持有控制权。Agent 角色是"帽子"——你在不同阶段戴不同的帽子，完成该阶段工作后切回 Orchestrator 帽子。

**Stage 5 特殊规则**: Pipeline Orchestrator 在 Stage 5.0 完成 Handoff Bundle 生成后，将控制权交给 `medical-pipeline` skill 的 orchestrator。medical-pipeline 接管 Stage 5.1-5.9 的调度，完成后返回 MSRA Pipeline Orchestrator 进行 Stage 6 Process Summary。
...
| 冲突类型 | 描述 | 典型场景 |
|---------|------|---------|
| **DATA_INCONSISTENCY** | 数据不一致 | Data Validator和Method Consultant对数据特征描述不一致 |
| **METHOD_DISAGREEMENT** | 方法选择分歧 | Method Consultant建议方法与Exec Inference验证结果冲突 |
| **RESULT_DIVERGENCE** | 结果差异 | Generator-Evaluator差异 |
| **QUALITY_GATE_FAILURE** | 质量门闸失败 | QC Inspector发现阻断项 |
| **RESOURCE_CONTENTION** | 资源竞争 | 多个Agent同时请求同一资源 |
| **PRIORITY_CONFLICT** | 优先级冲突 | 高优先级任务抢占资源 |
| **SEMANTIC_MISMATCH** | 语义不匹配 | 变量定义在Agent间理解不一致 |

### 冲突级别

| 级别 | 值 | 说明 | 处理策略 |
|------|---|------|---------|
| **TRIVIAL** | 1 | 轻微差异，记录即可 | 自动解决 |
| **MINOR** | 2 | 小差异，条件通过 | 自动解决 |
| **MODERATE** | 3 | 中等差异，需确认 | 投票决定 |
| **SIGNIFICANT** | 4 | 显著差异，阻断 | 仲裁决定 |
| **CRITICAL** | 5 | 严重差异，立即上报 | 用户决定 |

### 解决策略

```python
from agents.core import ConflictResolver, ConflictReport, ConflictLevel, ConflictType

resolver = ConflictResolver(message_bus)

# 注册投票者
resolver.register_voter("method_consultant", method_consultant_agent)
resolver.register_voter("exec_inference", exec_inference_agent)

# 设置用户回调
resolver.set_user_callback(ask_user_decision)
...
conflict = ConflictReport.create(
    ...
    description="missing_value_rate",
    evidence={"expected": "<5%", "actual": "15%"},
    suggestions=["继续", "重新验证"]
)
...
| 冲突类型 | 主要来源 | 备选 | 仲裁 |
|---------|---------|------|------|
| DATA_INCONSISTENCY | data_validator | method_consultant | qc_inspector |
| METHOD_DISAGREEMENT | method_consultant | exec_inference | qc_inspector |
| RESULT_DIVERGENCE | exec_inference | qc_inspector | orchestrator |
| QUALITY_GATE_FAILURE | qc_inspector | source_agent | user |
...
| 阶段 | 传递内容 | 禁止传递 | 节省比例 |
|------|---------|---------|---------|
| Stage 1 → 2 | 数据摘要 + 验证报告 + 清洗日志 | 原始数据表 | ~80% |
| Stage 2 → 3 | SAP + 分析规范书 + 变量构造定义 | EDA 原始图表 | ~60% |
...
| 规则 | 说明 | 违反后果 |
|------|------|---------|
| 单一控制源 | Orchestrator 始终持有控制权，Agent 不可自行调度 | Agent 自行调度 → 输出不可追溯 |
...
> 借鉴 MAS 综述的"cross-verify outputs"原则，确保每个 Agent 的输出至少被一个独立 Agent 验证。
