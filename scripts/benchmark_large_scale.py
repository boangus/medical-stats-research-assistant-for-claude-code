...
测试 Polars、DuckDB、Dask 在不同数据规模下的 CSV 读取和 groupby 聚合性能。
...
def run_benchmark(
    ...
    for n_rows in sizes:
        ...
        print(f"\n{'=' * 60}")
        print(f"  数据规模: {size_label} 行 ({n_rows:,} rows)")
        print(f"{'=' * 60}")
...
def print_summary(results: dict, sizes: list[int]) -> None:
    ...
    print(f"{'':>10} | {'Polars':>9} {'DuckDB':>9} {'Dask':>9} | {'Polars':>9} {'DuckDB':>9} {'Dask':>9}")
    print("-" * 80)
    ...
    for n_rows in sizes:
        ...
        p_c = f"{csv.get('polars', -1):9.3f}"
        d_c = f"{csv.get('duckdb', -1):9.3f}"
        dk_c = f"{csv.get('dask', -1):9.3f}"
        p_g = f"{grp.get('polars', -1):9.3f}"
