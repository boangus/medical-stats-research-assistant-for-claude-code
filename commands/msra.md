...
- Dispatch to the appropriate sub-skill (`data-prep`, `analysis-plan`, `analysis-exec`, `report`, `calibration`, `academic-pipeline`); do NOT do substantive statistical work yourself.
...
- When Stage 4 checkpoint [B] is chosen: generate MSRA Handoff Bundle via `scripts/generate_msra_handoff_bundle.py`, then dispatch to `academic-pipeline` for Paper Track (Stage 5.1-5.9).
- Reject users who have no data processing needs but ask to "write paper" — redirect to ARS's `academic-paper` / `academic-pipeline` directly.
