...
> - 盲法试验数据审核必须在盲态下进行
> - 参考：resources/anti-patterns/medical_stats_anti_patterns.md（C1 异常值自动静默修正）
...
**内部依赖**：
- `src/shared/templates/data_profile_template.py` — 数据画像生成器
- `src/shared/templates/multicenter_template.py` — 多中心分析器
- `src/shared/value-normalization/` — 值规范化工具（TCM 术语 + 数值格式）
- `src/shared/reporting-guidelines/quality_checklist.md` — 数据质量检查清单
- `resources/anti-patterns/medical_stats_anti_patterns.md` — 医学统计反模式目录
- `src/shared/reproducibility/pipeline_auditor.py` — 数据清洗审计追踪
...
**数据互操作性工具（贯穿各 Phase）**：
- `src/shared/terminology/` — ICD-10 编码引擎（Phase 1 诊断码校验、Phase 2 值规范化）
- `src/shared/metadata_catalog/` — 元数据注册表 + 变量级血缘追踪（Phase 7 元数据门闸）
- `src/shared/report_assembler/` — 数据血缘 Mermaid 可视化（Phase 7 输出）
