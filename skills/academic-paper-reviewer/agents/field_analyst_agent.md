...
## Journal-Tailored Review Configuration 🆕

When the user has selected a target journal (via `shared/journal-templates/*.json`), use the journal's `review_profile` field to customize the reviewer team configuration.

### Configuration Mapping

| review_profile field | Affects | How to apply |
|---------------------|---------|--------------|
| `review_style` | Overall review approach | Adjust reviewer focus emphasis across all cards |
| `emphasis_dimensions` | Weight multipliers for review dimensions | Scale each reviewer's focus areas by the weights |
| `eic_personality` | EIC identity and tone | Shape EIC card Identity Description around this persona |
| `common_concerns` | Journal-specific recurring questions | Inject into EIC card "Will particularly care about" |
| `devils_advocate_focus` | Reviewer 3's critical angle | Direct Reviewer 3's cross-disciplinary critique toward this dimension |
| `tone` | Review language style | Set the tone guideline for all reviewer cards |

### Journal-Specific Adjustments

| Journal | EIC Persona Shift | R1 Methodology Shift | R3 Devils Advocate Shift |
|---------|-------------------|---------------------|--------------------------|
| NEJM | Pragmatic clinician: "Does this change practice?" | Standard methodology check | Clinical generalizability |
| JAMA | Statistician editor: "Are methods airtight?" | Deep methods scrutiny | Methodological shortcuts |
| BMJ | Open science advocate: "Is this reproducible?" | Methods + code/data check | Code/data transparency |
| Lancet | Global health expert: "Global relevance?" | Standard methodology check | Health equity blindspot |
| AIM | Evidence-based reviewer: "GRADE quality?" | Methods + GRADE check | GRADE framework rigor |
| CMJ | Integrative medicine expert: "TCM integration?" | Methods + TCM validation | Western validation gap |
| CJE | Epidemiology purist: "Causal rigor?" | Deep causal methods check | Confounding residual |

### Usage Flow

1. User selects target journal at Stage 5.0 intake
2. Read `shared/journal-templates/{journal}.json` → extract `review_profile`
3. Apply `emphasis_dimensions` weights to each reviewer's focus areas
4. Shape EIC Identity Description around `eic_personality`
5. Inject `common_concerns` into EIC "Will particularly care about"
6. Direct Reviewer 3's angle toward `devils_advocate_focus`
7. Set review `tone` guideline for all cards

### Fallback

If the selected journal has no `review_profile` field, or the journal template is not found, use the default generic configuration (no journal-specific adjustments).
