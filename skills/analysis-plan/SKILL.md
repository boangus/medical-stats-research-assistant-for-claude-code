...
|----|--------|---------|------|----------------|-----------------|---------|
| CP-1 | Phase 1 EDA | EDA 完成 | 🟡 CHECKPOINT | EDA 报告覆盖全部 7 项必查内容；关键发现与方法选择建议已输出 | 任一必查项缺失（如未做缺失模式分析） | 补充缺失检查项后重新判定 |
...
| **建议纳入** | 单因素p<0.1 + 预后因素 | 统计+临床判断 | [p<0.1的非混杂变量] |
