...
在推荐事件驱动方法（Logistic/Cox/Poisson）之前，必须执行 EPV（Events Per Variable）可行性评估：

| 步骤 | 检查 | 阈值 | 动作 |
|------|------|------|------|
| 1 | 估计事件数 E | — | 从数据或用户输入获取 |
| 2 | 估计候选变量数 P | — | 从变量列表计数 |
| 3 | 计算 EPV = E/P | — | — |
| 4 | EPV ≥ 10 | 安全 | 正常推荐方法 |
| 5 | 5 ≤ EPV < 10 | 警告 | 推荐 Firth 惩罚似然，降级标记 |
| 6 | EPV < 5 | 阻断 | 推荐变量精简/合并，标记为探索性分析 |

> **与 Phase 3 的衔接**：此检查在 Phase 2 方法选择阶段执行，与 Phase 3 SAP 中的 SKIP 条件形成前置-后置双重保障。
> **事件数估计**：二分类结局取少数类事件数；生存分析取删失前事件数（failure events）；计数结局取总事件数。
> **候选变量数 P**：包括所有待评估的自变量（主效应 + 交互项 + 非线性项），不含截距和分层变量。
