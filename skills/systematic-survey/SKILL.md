...
|---|------|---------------------|---------------------|----------|
| **CP1** | Phase 1 后 | PICO 四要素齐全且可操作；纳入/排除标准具体（≥3 条纳入 + ≥2 条排除）；检索范围明确（≥3 个数据库 + 语言 + 时间段）；研究问题可在一个句子内回答 | PICO 缺要素；纳入标准不可操作（如"相关研究"）；未指定数据库或时间范围；研究问题模糊无法聚焦 | STOP → 退回 Phase 1 重新定义 PICO；最多 2 轮；仍失败 → 建议切换 quick-scan 先做文献摸底 |
...
- **FALLBACK**: 2 轮修正失败 → 优雅降级（标注局限性或建议模式切换）
...
| F1 分数 | 2 × (Precision × Recall) / (Precision + Recall) | 综合指标 | 召回率优先时 F1 可适当降低 |
...
| # | 失败场景 | 影响模式 | 严重度 | 处理策略 |
|---|---------|---------|--------|---------|
| F1 | PICO 问题过于宽泛（如"癌症的治疗"） | full, search-strategy | 高 | 退回 Phase 1，引导用户缩小范围：限定癌种、分期、干预类型、结局指标；最多 2 轮；仍失败 → 建议先做 quick-scan 摸底 |
| F2 | 检索结果过多（>10,000 篇）无法筛选 | full, quick-scan | 中 | 优化检索策略：增加限定词、使用更精确的 MeSH 词、限定研究类型/语言/年份；仍过多 → 分层筛选（优先核心期刊） |
| F3 | 检索结果过少（<10 篇） | full, search-strategy | 中 | 放宽检索条件：移除部分限制、增加同义词、扩展数据库、检索灰色文献；仍少 → 标注"文献基础薄弱"并建议修改 PICO |
| F4 | 全文获取率不足（<80%） | full | 中 | 尝试替代来源（机构库、作者联系、文献互助）；仍不足 → 标注"部分文献无法获取，可能存在选择偏倚" |
| F5 | 筛选一致性低（Kappa < 0.6） | full | 中 | 两名评审员讨论分歧 → 统一标准 → 重新筛选不一致部分；仍低 → 引入第三名评审员仲裁 |
| F6 | 纳入研究异质性过高（I² > 75%） | full, nma | 中 | 探索异质性来源（亚组分析、Meta 回归）；无法解释 → 仅做叙述性综合，不合并效应量 |
| F7 | 发表偏倚明显（漏斗图不对称 + Egger P < 0.1） | full, nma | 中 | 使用校正方法（Trim-and-fill、selection model）；仍显著 → 在讨论中强调发表偏倚的影响 |
| F8 | GRADE 评级全部为"低"或"极低" | full, grade-assessment | 低 | 完整报告降级原因 → 在结论中强调证据不确定性 → 建议未来研究方向 |
| F9 | 检索策略无法复现 | search-strategy | 严重 | 重新构建检索式并验证 → 确保每个步骤可追溯 → 提供完整检索日志 |
| F10 | 领域文献高度碎片化（无共识） | domain-report | 低 | 报告分歧现状 → 识别争议焦点 → 建议系统综述作为解决路径 |
