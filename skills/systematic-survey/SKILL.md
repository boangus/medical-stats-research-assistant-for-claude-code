...
| # | 失败场景 | 影响模式 | 严重度 | 处理策略 |
|---|---------|---------|--------|---------|
| F1 | PICO 问题过于宽泛（如"癌症的治疗"） | full, search-strategy | 高 | 退回 Phase 1，引导用户缩小范围：限定癌种、分期、干预类型、结局指标；最多 2 轮；仍失败 → 先做 quick-scan 摸底 |
| F2 | 检索结果过多（>10,000 篇）无法筛选 | full, quick-scan | 中 | 优化检索策略：增加限定词、使用更精确的 MeSH 词、限定研究类型/语言/年份；仍过多 → 分层筛选（优先核心期刊） |
| F3 | 检索结果过少（<10 篇） | full, search-strategy | 中 | 放宽检索条件：移除部分限制、增加同义词、扩展数据库、检索灰色文献；仍少 → 标注"文献基础薄弱"并退回 Phase 1 修改 PICO |
...
| F8 | GRADE 评级全部为"低"或"极低" | full, grade-assessment | 低 | 完整报告降级原因 → 在结论中强调证据不确定性 → 明确列出未来研究方向 |
| F9 | 检索策略无法复现 | search-strategy | 严重 | 重新构建检索式并验证 → 确保每个步骤可追溯 → 提供完整检索日志 |
| F10 | 领域文献高度碎片化（无共识） | domain-report | 低 | 报告分歧现状 → 识别争议焦点 → 触发 full 模式系统综述作为解决路径 |
