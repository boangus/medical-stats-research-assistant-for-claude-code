...
> **以下行为必须避免**。违反任何一条将导致资源浪费或误导用户。

### 🚫 搜索禁忌

| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 1 | 使用 `--raw` 获取完整论文做初步了解 | 浪费大量token(10-50k)，且效率低 | 始终先用 `--brief` 筛选 |
| 2 | 批量读取完整论文不设置token预算 | 快速耗尽API限制 | 先用 `--limit 5 --format json` 批量获取元数据 |
| 3 | 跳过 `--head` 直接读取全文 | 无法定位关键信息位置 | 先 `--head` 了解结构，再针对性读取 |

### 🚫 读取禁忌

| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 4 | 用 `--preview` 代替明确指定section | preview是模糊的10k字符，不够精准 | 使用 `--section Introduction` 等精确指定 |
| 5 | 不检查token余额就进行批量操作 | 可能在中途耗尽余额，导致任务中断 | 批量操作前先 `deepxiv token` 检查余额 |
| 6 | 对付费期刊使用deepxiv | 无法访问，结果为空 | 确认论文是否为arXiv/PMC开源论文 |

### 🚫 Agent使用禁忌

| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 7 | 用agent做简单的事实查询 | agent成本高(5-20k tokens)，浪费资源 | 用 `--brief` 或 `--section` 直接获取 |
| 8 | agent多轮对话不设max-turn限制 | 可能无限循环，消耗大量token | 设置 `--max-turn 5-10` 限制对话深度 |
| 9 | 不配置agent就使用 | 缺少配置会导致分析质量不稳定 | 先 `deepxiv agent config` 配置LLM后使用 |
...
| 触发条件 | 一线处理 | 仍失败兜底 |
|---------|---------|-----------|
| `Paper Not Found` (arXiv ID无效) | 检查ID格式(应为2409.05591格式)，在https://arxiv.org验证 | 告知用户"论文未找到"，提供搜索建议 |
| `RateLimitError: Daily limit reached` | 等待次日重试，或发送邮件申请提高限额 | 使用其他论文ID继续，跳过受限部分 |
| `AuthenticationError: Invalid token` | 运行 `deepxiv config --token YOUR_NEW_TOKEN` | 删除 `~/.env` 重新注册新token |
| `Request timed out` (大型论文) | 改用 `--brief` 或 `--section` 分段读取 | 使用Python API设置 `Reader(timeout=180)` |
| 搜索结果为空 | 尝试调整关键词、使用分类过滤 | 告知用户，建议使用更通用的搜索词 |
| PMC论文不可用 | 确认PMC ID格式(PMCxxxxxx)，检查是否开源 | 告知用户该论文不在开源范围内 |

### 检查点

- **[MANDATORY] 批量操作前**: 必须检查token余额 `deepxiv token`，确认足够完成操作
- **[SLIM] 新论文理解**: 始终先用 `--brief` 获取TLDR，确认相关性后再深入
- **[ADAPTIVE] token接近限制时**: 自动切换到轻量模式(--brief优先)，避免任务中断
