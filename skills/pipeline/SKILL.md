...
- **阻断检查清单 (8 项)**:
  ...
  - **3+ 项未通过 或 项目 5/6/7 未通过** → ❌ **强制退回 Stage 1 修订**
...
| M2 | Stage 1.5 通过后 | 数据门闸结果 | 7 项门闸检查结果 + 阻断判定 | 通过继续 / 退回修订 / 带条件通过 |
| M3 | Stage 2.5 通过后 | SAP 门闸结果 | 7 项门闸检查结果 + 阻断判定 | 通过继续 / 退回修订 / 带条件通过 |
| M4 | Stage 3.5 通过后 | 结果门闸结果 | 8 项门闸检查结果 + 阻断判定 | 通过继续 / 退回修正 / 带条件通过 |
| M5 | Stage 4 完成时 | 合规检查 | 规范合规报告 + 最终 checklist | 通过交付 / 修改 / 标记草稿 |
| M6 | 回退循环超限时 | 同阶段回退 ≥ 2 次 | 收敛检测报告 + 偏差记录 | 接受风险继续 / 强制停止 |
...
| 触发条件 | 一线处理 | 仍失败兜底 |
|---------|---------|-----------|
| 数据验证发现严重问题 | 回到 Stage 1 重新清洗 | 标记数据为"不可用"，建议重新收集 |
...
| 质检发现严重错误 | 回到 Stage 3 修正分析 | 标记结果为"待验证"，不建议进入报告阶段 |
...
**Early-stopping 辅助判断**：
- 如果质量门闸未通过项数在上一次回退后**没有减少** → 建议用户暂停，换人审查或换方法
- 如果未通过项数减少但仍有 **≥3 项关键项未通过** → 建议用户考虑修改研究设计
