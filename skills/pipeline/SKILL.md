...
- **Checkpoint**: 仅 [MANDATORY-ME1] 主要分析结果确认（Phase 3 完成后）。其余阶段无确认点。
...
| M3 | Stage 3.5 通过后 | 结果门闸结果 | 14 项门闸检查结果 + 阻断判定 | 通过继续 / 退回修正 / 带条件通过 |
...
| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 18 | 在高效模式下跳过 MANDATORY 检查点 | MANDATORY 是所有模式的硬性要求，SLIM 和 ADAPTIVE 才可按模式跳过 | 5 个 MANDATORY 检查点（M1-M5）在任何模式下都必须展示并等待用户决策 |
