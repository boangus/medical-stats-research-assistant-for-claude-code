...
depends_on: [data-prep, analysis-plan, analysis-exec, report, calibration, academic-paper-reviewer, deep-research]
...
│  Estimands(估计目标) → 方法探讨 → SAP → 审查 → 变量构造定义  │
...
│  → 描述性统计(Table 1) → 依从性分析 → 安全性分析             │
│  → 主要分析 → 敏感性分析 → 亚组分析 → 假设检验 → 质检        │
└────────────────────────────┬─────────────────────────────────┘
                             ▼
┌──────────────────────────────────────────────────────────────┐
...
└────────────────────────────┬─────────────────────────────────┘
                             ▼
          ┌────────────────────────────────────────────┐
          │ ★ STAGE 4 CHECKPOINT (MANDATORY-GATE-05)   │
...
> **目的**：根据用户经验和需求，提供引导模式与专家模式的无缝切换。
> **触发时机**：Pipeline 启动时自动询问，或用户通过参数指定
> **核心原则**：两种模式下的分析结果一致性与准确性不受影响

### 2.2.1 模式定义

| 维度 | 引导模式 (Guided) | 专家模式 (Expert) |
|------|-------------------|------------------|
| **目标用户** | 首次使用/不熟悉统计分析流程的用户 | 熟悉系统、有统计分析经验的用户 |
| **交互深度** | 分步骤详细指引，每步单独确认 | 批量操作，减少不必要交互 |
| **概念解释** | 显示每个术语的含义、参数说明、决策建议 | 仅显示操作选项，不显示概念解释 |
| **默认值** | 提供推荐默认值并说明理由 | 提供默认值但不解释 |
| **错误处理** | 详细解释错误原因和修复建议 | 简洁错误信息+修复选项 |
| **跳过选项** | 不允许跳过关键步骤 | 可跳过非关键步骤 |
| **快捷操作** | 无 | 支持快捷键和批量操作 |
...
```
── MSRA Pipeline 启动 ──
检测到: 研究类型={RCT/观察性/诊断} | 入口={Stage N}

选择操作模式:
  [1] 引导模式 (Guided) — 分步骤详细指引，含概念解释和决策建议
  [2] 专家模式 (Expert) — 快速操作，减少交互，支持批量处理

输入编号 (默认: 1):
```

**模式持久化**：用户选择的模式记录在 passport.json 中，后续阶段自动继承：
```json
{
  "operation_mode": "guided"  // 或 "expert"
}
```

**模式切换**：用户可随时通过以下方式切换：
- 输入 `/mode guided` 或 `/mode expert`
- 在任何 Checkpoint 选择"切换模式"
...
| 步骤 | 引导模式 | 专家模式 |
|------|---------|---------|
| 数据验证 | 逐项展示验证结果，解释每项含义 | 汇总展示，仅标记异常项 |
| 清洗策略 | 逐步引导：先缺失→再异常→最后逻辑 | 一次性展示全部清洗建议，批量确认 |
| 值规范化 | 解释每种变体类型和规范化规则 | 仅展示变体数量和规范化结果 |
| EDA报告 | 逐节解读EDA结果，解释统计含义 | 直接输出完整EDA报告 |
| 盲态审核 | 详细解释审核流程和判定标准 | 直接执行审核，输出结果 |
| 数据库锁定 | 解释锁定含义和不可逆性 | 直接锁定，输出确认 |

#### Stage 2: 分析计划

| 步骤 | 引导模式 | 专家模式 |
|------|---------|---------|
| 数据要素确认 | 逐项确认暴露/结局/协变量，含概念解释 | 一次性展示全部要素，批量确认 |
| Estimands | 解释5个属性的含义和选择依据 | 仅展示属性表格，用户填写 |
| 方法选择 | 展示决策树，解释每种方法的适用场景和优劣 | 直接推荐方法，用户确认或修改 |
| SAP制定 | 分章节制定，每章确认后继续 | 使用模板快速生成，一次性确认 |
| SAP审查 | 逐项解释审查结果和修改建议 | 汇总展示审查结果，批量修改 |

#### Stage 3: 分析执行

| 步骤 | 引导模式 | 专家模式 |
|------|---------|---------|
| 代码生成 | 逐段生成代码，解释每段功能 | 一次性生成全部代码 |
| 执行监控 | 实时展示执行进度和中间结果 | 仅展示最终结果和错误 |
| 结果解读 | 逐表逐图解读，解释统计含义 | 直接输出全部结果 |
| 质量检查 | 逐项展示检查结果，解释异常 | 汇总展示，仅标记异常 |

#### Stage 4: 报告生成

| 步骤 | 引导模式 | 专家模式 |
|------|---------|---------|
| 结果解读 | 逐项解读主要/次要/安全性结果 | 汇总展示关键发现 |
| 表格图表 | 逐表逐图确认格式和内容 | 批量生成，一次性确认 |
| 方法学描述 | 逐步引导撰写方法学段落 | 使用模板快速生成 |
| 报告组装 | 分章节确认后组装 | 一次性组装完整报告 |
...
```
── Estimands 确认 ──
💡 Estimands（估计目标）: 描述治疗效果的精确框架，包含5个属性。

| 属性 | 含义 | 你的选择 |
|------|------|---------|
| 治疗 (Treatment) | 比较的干预 vs 对照 | [{选择}] |
| 人群 (Population) | 目标研究人群 | [{选择}] |
| 变量 (Variable) | 感兴趣的结局指标 | [{选择}] |
| 间发事件 (Intercurrent Events) | 可能影响结局的后续事件 | [{选择}] |
💡 间发事件: 如退出、抢救治疗、死亡等，需要选择处理策略
| 汇总 (Summary) | 人群层面的效果度量 | [{选择}] |
...
💡 推荐: ANCOVA（调整基线HbA1c）
理由:
  1. RCT连续终点，ANCOVA是标准方法
  2. 调整基线可提高效能（减少方差约30%）
  3. 满足正态性假设（Shapiro-Wilk p=0.45）
  4. 满足方差齐性（Levene p=0.62）
...
── 样本量计算 ──
💡 参数说明:
  - α (显著性水平): 控制假阳性率，通常设0.05
    → 含义: 有5%的概率在无效时错误地声明有效
  - Power (检验效能): 控制假阴性率，通常设0.80
    → 含义: 有80%的概率在有效时正确地声明有效
  - 效应量: 预期的治疗效果大小
    → 来源: 文献/预实验/MCID
  - 失访率: 预计丢失的受试者比例
    → 建议: 通常设10-20%

当前参数: α=0.05, Power=0.80, 效应量=0.3, 失访率=15%
计算结果: 需要 N=200 人

调整参数? [α / Power / 效应量 / 失访率 / 确认]
```
...
数据要素:
  ...
  [x] 协变量: age, sex, bmi, baseline_hba1c
  [x] 分层: center_id
  [x] 时间: follow_up_days

分析计划:
  [x] 主分析: ANCOVA(调整baseline_hba1c)
  [x] 敏感性: MI + Tipping Point
  [x] 亚组: age(<65/≥65), sex
...
| 快捷命令 | 功能 | 等价操作 |
|---------|------|---------|
| `/quick` | 切换到专家模式+Quick模式 | `/mode expert` + 跳过非关键步骤 |
| `/guided` | 切换到引导模式 | `/mode guided` |
| `/accept-all` | 批量确认所有推荐 | 逐项确认的快捷方式 |
| `/skip-eda` | 跳过EDA详细展示 | 仅输出EDA摘要 |
| `/template` | 使用标准模板快速生成SAP | 跳过方法讨论，直接用模板 |
| `/batch` | 批量执行多个分析 | 逐个执行的快捷方式 |
...
```
── Stage 2 完成 ──
SAP: approved ✅ | 方法: ANCOVA+MI | 亚组: 2个 | 敏感性: 3种
变量: 12个(强制4+建议5+排除3) | 门闸: PASS(8/8)
→ Stage 3 [继续]
```

### 2.2.6 模式一致性保证

**IRON RULE**: 两种模式下的分析结果必须完全一致。

| 保证措施 | 说明 |
|---------|------|
| 相同的统计方法 | 模式只影响交互方式，不影响方法选择 |
| 相同的参数默认值 | 两种模式使用相同的默认参数 |
| 相同的质量门闸 | 门闸检查标准不因模式而异 |
| 相同的产物格式 | 输出文件格式和内容完全一致 |
| 模式切换可追溯 | passport.json 记录模式切换历史 |
...
| 交互深度 | 最深 | 最浅 | 最浅 |
| 概念解释 | 有 | 无 | 无 |
| 步骤跳过 | 不允许 | 允许非关键 | 允许大部分 |
| 模板使用 | 不使用 | 可选 | 强制使用 |
| 适用场景 | 学习/首次使用 | 日常分析 | 快速出结果 |

> Quick模式是专家模式的子集，额外跳过文献检索、DAG预览等附属产物。
...
- **子步骤**:
  ...
  5. EDA 数据质量检查（缺失模式、异常值、分布特征——仅用于数据质量评估，不用于方法选择）
...
- **Checkpoint**: [MANDATORY-GATE-01] 门闸通过后进入 Stage 2
...
- **子步骤**:
  ...
  5. 变量构造定义（Phase 5: 所有分析变量的构造公式、切点、缺失处理预定义，含 DAG 预览附属产物）
- **🆕 多数据集模式**（仅 multi-dataset 模式）：Phase 3 条件章节: 多中心分析计划，制定中心效应处理策略
- **Checkpoint**: 仅 [MANDATORY-GATE-02] SAP 门闸检查。Stage 2 内部无额外确认点。
...
- **Checkpoint**: [MANDATORY-GATE-02] 门闸通过后进入 Stage 3
...
- **🆕 多数据集模式**（仅 multi-dataset 模式）：Phase 3 条件子步骤: 多中心汇总分析，含森林图 + 异质性检验 + leave-one-out
- **Checkpoint**: 仅 [MANDATORY-GATE-03] 主要分析结果确认（Phase 3 完成后）。其余阶段无确认点。
- **🆕 SAP Amendment**: Phase 3 异常处理分支支持透明的 SAP 修正流程（最多 3 次，详见 analysis-exec/SKILL.md Phase 3）
...
- **Checkpoint**: [MANDATORY-GATE-03] 门闸通过后进入 Stage 3.7
...
- **子步骤**:
  ...
  7. 报告组装（Phase 6: JSON骨架→HTML图文报告，按模板章节组织）
- **Checkpoint**: [MANDATORY-GATE-04] 统计质量检查 → 确认 → [MANDATORY-GATE-05] ★ STAGE 4 CHECKPOINT: [A] 结束 / [B] Paper Track
...
| GATE-04 | Stage 4 完成时 | 统计质量检查 + 报告交付决策 | 统计质量报告 + ★[A]结束/[B]继续Paper Track | [A]结束 / [B]进入Stage 5.0 |
| GATE-06 | 任一门闸回退≥3次（收敛失败） | 收敛检测触发 | 未通过项清单 + 回退历史 + 风险评估 | 书面接受风险继续 / 退回根本原因排查 / 换方法换人 |
...
```
── 协作深度观察（非阻断）──
委托强度: 4/5 ✅ | 认知警觉: 2/5 ⚠️ 必须在门闸结果上逐项核对 | 认知再分配: 3/5
```
...
| # | 位置 | 提示模板 | 备注 |
|---|------|---------|------|
| PIPE-01 | Stage 2 Estimands + 方法确认后 | `Estimands + 方法已确认, 生成 SAP? [y/N]` | 详细引导模式下执行 |
| PIPE-02 | Stage 3 Phase 3 主要分析完成后 | `主要分析结果已出，继续敏感性分析? [y/N]` | 详细引导模式下执行 |
| PIPE-03 | Stage 4 Phase 1 结果解读完成后 | `结果解读已完成，继续生成表格? [y/N]` | 详细引导模式下执行 |
| PIPE-04 | Stage 4 Phase 4 方法学描述完成后 | `方法学描述已完成，进入统计质量检查? [y/N]` | 详细引导模式下执行 |
...
> - [Exec Runner Agent](../../agents/exec_runner_agent.md) — Stage 3 Phase 0-3
> - [Exec Inference Agent](../../agents/exec_inference_agent.md) — Stage 3 Phase 4-6
...
Stage 3 使用 Generator-Evaluator 双角色：Phase 0-3 为 Exec Runner，Phase 4-6 为 Exec Inference。
...
```
第 1 次退回 → 正常修改，记录偏差原因
第 2 次退回 → 警告："已退回 2 次，必须检查根本原因或降低标准"
第 3 次退回 → BLOCK: "同阶段已退回 3 次，自动转入偏差记录模式"
                → 必须走 GATE-06 Checkpoint，用户需书面接受风险
...
| Skill 依赖缺失 | 所需 skill（如 academic-paper-reviewer）未安装 | 跳过对应阶段（如 Paper Track），提示用户安装 | [SKIP: dependency missing] |
...
| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 15 | 门闸回退时不更新回退计数器 | 收敛检测依赖于准确的回退计数，不更新会导致门闸无限循环 | 每次回退必须自增计数器，门闸消费后更新 |
| 16 | 多阶段错误同时发生时从最后一个出错阶段修复 | 下游阶段的问题可能是上游问题带入的，下游修复无法根治 | 从最早出错的阶段开始修复，固定一个再处理下一个 |
| 17 | 异常发生时静默继续不告知用户 | 用户失去对意外情况的知情权和决策权 | 所有异常（包括 ADAPTIVE 级别的）必须先展示并得到用户响应才继续 |

### 🚫 检查点协议禁忌 🆕

| # | 禁止行为 | 为什么 | 正确做法 |
|---|---------|--------|---------|
| 18 | 在高效模式下跳过 MANDATORY 检查点 | MANDATORY 是所有模式的硬性要求，SLIM 和 ADAPTIVE 才可按模式跳过 | 6 个 MANDATORY 检查点（GATE-01~GATE-06）在任何模式下都必须展示并等待用户决策 |
| 19 | 多个 MANDATORY 检查点同时触发时只展示最新的 | 当前阶段的信息不足以让用户做出跨阶段的风险决策 | 同时触发的 MANDATORY 检查点必须依次展示，上一点决策后再展示下一点 |
...
**状态转换规则**：
- Stage 5.2 → user confirmation → 5.3
- Stage 5.3 → PASS → 5.4 (FAIL → fix and re-verify, max 3 rounds)
- Stage 5.4 → Accept → 5.8 / Minor|Major → 5.5 / Reject → 5.2 or end
- Stage 5.5 → user confirmation → 5.6
- Stage 5.6 → Accept|Minor → 5.8 / Major → 5.7
- Stage 5.7 → user confirmation → 5.8
- Stage 5.8 → PASS (zero issues) → 5.9 (FAIL → fix and re-verify)
- Stage 5.9 → MD → DOCX via Pandoc → PDF → end
...
| 4 | `collaboration_depth_agent` | Observer (advisory only — never blocks). Score user-AI collaboration pattern | FULL/SLIM checkpoints |
| 5 | `claim_ref_alignment_audit_agent` | Opt-in claim faithfulness auditor. Audit citations for claim ↔ reference alignment | Optional |

### 11.6 Cross-Session Resume (Opt-in)

Set `ARS_PASSPORT_RESET=1` to promote FULL checkpoints to context-reset boundaries. Use `resume_from_passport=<hash>` in a fresh session to continue from the recorded stage.

**Gate (emit)**: `ARS_PASSPORT_RESET=1` must be set in the emitting session.
...
3. Quality check: Is output of [Current] ≥ [Previous]? If not, PAUSE.
...
```
═══════════════════════════════════════
  MSRA Paper Track Progress
═══════════════════════════════════════
  Stage 5.0 [Paper Intake]      ✅ Done
  Stage 5.1 [Literature]        ✅ Done
  Stage 5.2 [Writing]           🔄 Current
  Stage 5.3 [Integrity]         ⏳ Pending
  Stage 5.4 [Review]            ⏳ Pending
  Stage 5.5 [Revise]            ⏳ Pending
  Stage 5.6 [Re-review]         ⏳ Pending
  Stage 5.7 [Re-revise]         ⏳ Pending
  Stage 5.8 [Final Integrity]   ⏳ Pending
  Stage 5.9 [Finalize]          ⏳ Pending
═══════════════════════════════════════
```
...
| 8 | 收敛检测触发后仍继续退回而不转入偏差记录模式 | 无限循环退回浪费资源，且无法推进流程 | 回退 ≥ 3 次后必须走 GATE-06 Checkpoint，转入偏差记录模式 |
