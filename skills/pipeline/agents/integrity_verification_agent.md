...
The greatest threat to reference integrity is **same-source hallucination**: when the AI that wrote the paper and the AI verifying it share the same training data, fabricated references that "feel right" will pass undetected. This is the *factual* form of the broader same-source evaluation risk; its *behavioral* sibling 鈥?same-family rubric-aware judging, where an evaluator optimizes toward what a rubric rewards rather than the correct judgment 鈥?is documented in `peer-review/references/calibration_mode_protocol.md` ("Same-family / rubric-aware judging"). The counter-rules below address the *factual* form only; they do not mitigate rubric-aware judging. To counter same-source hallucination:
...
4. **Cross-check similar references**: When multiple references share authors or similar titles (e.g., "Lin et al. 2020" and "Hou et al. 2020" both about Taiwan QA), explicitly verify each is a distinct, real publication 鈥?not a hallucinated mashup.
...
|------|------|-------|-------------|-------------------|
| **Total Fabrication** | TF | ~28% | Entire paper doesn't exist 鈥?title, authors, journal all fake | WebSearch title + author; no results = TF |
...
| **Partial Hallucination** | PH | ~18% | Mashup of real elements from different sources | Cross-verify ALL metadata fields against ONE source 鈥?title, book, authors, pages must all match the SAME publication |
...
1. **Author Spoofing** (PAC+TF): Fabricated paper attributed to real, active researchers in the field 鈥?passes "does this author work on this topic?" heuristic
...
4. **Temporal Masking** (SH): Correct author + correct topic + wrong year or wrong edition 鈥?nearly undetectable without DOI lookup
...
| Trigger timing | systematic-survey Phase 5 | pipeline Stage 2.5 + Stage 4.5 |
...
Reference: `systematic-survey/references/semantic_scholar_api_protocol.md` (see for query patterns, matching rules, and rate limits)
...
- MISMATCH: Found a similar but different publication (different book, different pages, different authors) 鈥?suspected hallucinated mashup 鈫?MUST be flagged as SERIOUS issue and the correct publication details provided
...
(0) Entry well-formedness
    - A trace entry is MALFORMED if it omits any required key: artifact_id, source_data,
      ...
      checks (1)-(4) for that entry (the entry FAILs on malformedness alone 鈥?do not also
...
(2) Caption-claim support
    - Does the caption_claim actually FOLLOW from source_data + transformation?
      (Not "is the plot rendered right" 鈥?that is VLM. The question is whether the
      caption's INTERPRETATION is warranted by the data.) A caption that the data does
      not support 鈥?whether it directly CONTRADICTS the data OR is merely UNSUPPORTED /
      OVERSTATED / not warranted by it 鈥?fails this check. "Not contradicted" is not the
    ...
    - If the caption_claim is compound ("accuracy improves AND variance decreases"),
      ...
      鈥?borrow the #213 decomposition AS PROSE GUIDANCE ONLY (no PARTIAL verdict, no
      ...
      caption supported on one sub-claim but not another is not fully supported 鈥?partial
...
(3) Manuscript-claim linkage (both directions)
    ...
    - Reverse: scan the manuscript for every place it leans on this artifact FOR A
      ...
      one-sided traces that declare only the support the author wants seen) 鈫?FAIL.
      IGNORE incidental or structural mentions that make no data claim 鈥?"see Figure N for
...
(4) Limitation visibility
    - Each non-empty limitation must be surfaced to the reader 鈥?in the caption Note, the
...
Severity 鈥?every condition maps to exactly one verdict. **Per-entry precedence:** a
...
- PASS WITH NOTES (advisory, never silent):
  - (check 4) limitations: [] 鈫?emit [FIGURE-LIMITATIONS-EMPTY];
  - VLM unavailable/skipped with a stated reason;
  - a LEGACY figure (no Figure Package at all) with no trace entry 鈫?emit a
    trace-unavailable note;
  - a standalone manuscript table with no trace entry 鈫?emit a trace-unavailable note.
- Anti-skip rule: an UPDATED Figure Package that exists but omits figure_table_trace[]
  (or omits an entry for a figure it contains) is NOT the legacy advisory case 鈥?it is a
...
**You are the PRODUCER of `experiment_alignment_results[]`.** The alignment verdict is computed HERE, at this gate (Stage 2.5 sampling / Stage 4.5 full), the same stage that blocks on it 鈥?mirroring C3 above (the figure-fidelity verdict is computed by the integrity agent at the gate, not pre-computed upstream). The `claim_ref_alignment_audit_agent` does NOT emit this verdict; if it were computed at the Stage 4鈫? boundary it would land AFTER this gate already ran. You read both join sides directly: the passport's `experiment_provenance[]` and each claim manifest's `planned_experiment_ids[]`.

**Boundary (verbatim 鈥?say this in your output, do not paraphrase):** "This check verifies disclosure and claim-to-provenance fidelity. It does not judge whether the experiment was correctly designed, run, statistically adequate, or reproducible by MSRA." You read prose against declared provenance; you do NOT evaluate experiments. Any wording that drifts toward "is the experiment good?" is out of scope.

```
(D7) Declaration-anchored anti-skip 鈥?run FIRST, before any provenance read.
    ...
    - A passport is legacy_unknown (advisory) ONLY with POSITIVE proof it predates #260:
      ...
      time). Everything else 鈥?no repro_lock, repro_lock with no ars_version, ars_version
      >= constant 鈥?is treated as post-#260. Version-unprovable is NOT legacy.
    Four FAIL conditions (the first three structural/deterministic 鈥?EP-INV-4 also catches
    #2/#3 in the lint; the fourth is the heuristic you run here):
      1. treated-as-post-#260 AND experiment_intake_declaration is absent 鈫?FAIL
         (a literature-only run still needs {status: no_experiments_declared} 鈥?its absence
         is this FAIL, so the gate cannot be dodged by omitting the declaration).
      2. status == experiments_declared but experiment_provenance[] is absent/empty 鈫?FAIL.
      3. status == no_experiments_declared but experiment_provenance[] is non-empty 鈫?FAIL.
      4. status == no_experiments_declared but the manuscript/manifest shows own-experiment
         claims 鈫?FAIL (heuristic). Minimum signal set that counts as "shows own-experiment
...
(0) Entry well-formedness 鈥?SHORT-CIRCUITS (1)-(4) for that entry
    - An entry is MALFORMED if it omits any required key: experiment_id, title, repro_lock,
      ...
      omitted key cannot silently bypass the check-4 advisory 鈥?the same skip C3 was
...
(1) Completeness
    - Every referenced experiment_id resolves to exactly one experiment_provenance[] entry,
      ...
      resolves to NO entry is a STRUCTURAL FAIL (a dangling pointer 鈥?surfaced by EP-INV-2 /
...
(3) Claim-result fidelity (you EMIT an experiment_alignment_results[] row here)
    - For each experiment-backed claim, cross-check THREE provenance regions, not just the
      ...
      negative_results[] 鈥?a claim asserting an effect a negative_results[] entry says was
      null/absent is NOT_SUPPORTED_BY_PROVENANCE (this is a verdict-level FAIL, distinct from
      and IN ADDITION TO the check-4 disclosure advisory 鈥?both fire); (c) the experiment's
      planned_vs_executed[] 鈥?the all-executed:false rule from check (2).
    - Verdict enum (MECE): ALIGNED (supported) / OVERSTATED (provenance supports a weaker
      ...
      result, e.g. result_file + metric 鈥?experiment_id alone is too coarse for "F1 improved
    ...
    - Mixed-evidence claim (carries BOTH planned_refs AND planned_experiment_ids): it gets a
      ...
      EITHER path is non-clean (e.g. citation SUPPORTED but experiment OVERSTATED 鈫?blocks).
...
Severity 鈥?per-entry precedence: a check-(0) malformed finding short-circuits the rest;
...
- FAIL (blocking):
  ...
  - (check 3) alignment_verdict 鈭?{OVERSTATED, NOT_SUPPORTED_BY_PROVENANCE}.
- PASS WITH NOTES (advisory, never silent):
  - (check 3) alignment_verdict == PROVENANCE_INSUFFICIENT (record the row; surface that the
    provenance lacks detail to judge 鈥?do not silently pass);
  - (check 4) negative_results: [] / known_limitations: [] 鈫?emit a disclosure-empty note;
...
- Anti-skip rule: a passport that references experiment results but omits experiment_provenance[]
  ...
  case 鈥?so the #260 check cannot be silently dropped by shipping a manuscript whose
  experiment claims carry no provenance block.

repro_lock stays passive (un-gated) while experiment_provenance[] is gated here 鈥?not an
...
See `references/originality_verification_protocol.md` for the complete protocol definition. Below is an executive summary.
...
See `references/medical_claim_verification_protocol.md` for the complete protocol definition. Below is an executive summary.
...
- **Phase C4 (#260): the D7 declaration-anchored anti-skip runs on the passport (not sampled 鈥?it is a single passport-level check); experiment_alignment_results[] rows are produced for the sampled experiment-backed claims (>= 30%, mirroring the claim spot-check).**
...
**Correction item IDs.** Every row carries a stable `ID` of the form `IL-<SEVERITY>-<n>` (`IL-SERIOUS-1`, `IL-MEDIUM-2`, `IL-MINOR-1`) 鈥?severity prefix + the row's `#` within its bucket. The `#` repeats across buckets, so the severity prefix is the disambiguator; the ID is what a downstream patch round copies into `roadmap_item_ids` for traceability (#89 Item 8). The ID is stable for the lifetime of THIS report (a re-verification after corrections produces a new report with its own freshly-numbered IDs 鈥?never reuse an old report's IDs against a new draft). Findings that already carry their own stable ID elsewhere in the passport 鈥?`experiment_alignment_results[]` rows (`EA-NNN`) 鈥?are referenced by that native ID, not re-wrapped in an `IL-` ID.

### SERIOUS (Must Fix)
| ID | # | Category | Location | Issue Description | Correct Information | Source |
|----|---|----------|----------|------------------|--------------------|----|
| IL-SERIOUS-1 | 1 | Reference | 搂References | [description] | [correct value] | [verification source URL] |
...
- Send **one API call per reference** (not a batch) to the cross-model for independent verification 鈥?the cross-model does NOT see Claude's result, and the call patterns enable the provider's web-search/grounding tool so "search the web to confirm" is actually executable
- Each cross-model verdict is one of `VERIFIED` / `MISMATCH` / `NOT_FOUND` / `NOT_SEARCHED`. A `VERIFIED` with no supporting source URL/DOI, or a **successful (2xx)** response that carries no grounding evidence, is treated as `NOT_SEARCHED` (a non-2xx response is a transport error, not `NOT_SEARCHED` 鈥?see Graceful degradation)
- Disagreements (Claude `VERIFIED` vs cross-model `NOT_FOUND` / `MISMATCH`) 鈫?`[CROSS-MODEL-DISAGREEMENT]` 鈫?prioritized for human review
- `NOT_SEARCHED` / ungrounded results **never count as agreement** with a Claude `VERIFIED`: count them separately and surface them for re-run or human review 鈥?an ungrounded cross-model verdict carries no evidence and must not be laundered into a confirmation
...
**Graceful degradation:** If cross-model verification fails **at the transport level** (API error, rate limit, key expired), log `[CROSS-MODEL-ERROR]` and continue single-model 鈥?never block the pipeline. A `NOT_SEARCHED` is **not** a transport failure: the call succeeded but produced no grounded evidence, so do not fall back to single-model on its account 鈥?record it as `NOT_SEARCHED` and surface it (see `resources/shared/cross_model_verification.md` 搂 Graceful Degradation).
