...
- **Style Profile consumption** (optional) — If a Style Profile is available from report (论文写作模式) intake, the report compiler applies it as a soft guide for the Executive Summary and Synthesis sections. Discipline conventions and report objectivity take priority.
- **Writing Quality Check** — The report compiler runs a writing quality checklist before finalizing: flags AI-typical overused terms, checks sentence/paragraph length variation, removes throat-clearing openers. See `../../resources/academic-paper/references/writing_quality_check.md`.
...
|----------|-------------|
| Writing a paper (not researching) | `report` (论文写作模式) |
| Reviewing a paper (structured review) | `academic-paper-reviewer` |
| Full research-to-paper pipeline | `pipeline` (Stage 5 Paper Track) |
...
**Mode A — orchestrator-driven (default):** `pipeline_orchestrator_agent` (in `pipeline` skill §11.5) runs all phases end-to-end with state tracking via Material Passport.

**Mode B — phase-by-phase (cross-session resume):** User invokes one agent per phase across sessions for long-running projects. Common pattern via `ARS_PASSPORT_RESET=1` + `resume_from_passport=<hash>` (see `../../resources/academic-pipeline/references/passport_as_reset_boundary.md`).
...
## Handoff Protocol: deep-research → report (论文写作模式)

After research is complete, the following materials can be handed off to `report` (论文写作模式):
...
`report`'s `intake_agent` will automatically detect available materials and skip redundant steps:
...
See `pipeline/SKILL.md §11 Paper Track 扩展协议` for the complete workflow.
...
deep-research + report(论文模式)          -> Full research-to-publication pipeline
deep-research (socratic) + report(plan) -> Guided research + paper planning
deep-research (systematic-review) + report(论文模式) -> PRISMA systematic review paper
```
