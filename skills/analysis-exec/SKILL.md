...
│  │  ┌──────────────────────────────────┐    │          │
│  │  │ Phase 0: 样本量验证               │    │          │
│  │  │ Phase 1-3: SAP解析+环境准备       │    │          │
│  │  │ Phase 4: 变量构造                 │    │          │
│  │  │ Phase 5: 描述统计                 │    │          │
│  │  │ Phase 6: 推断分析 (Hybrid Prompt) │    │          │
│  │  │   ┌─────────────────────────┐     │    │          │
│  │  │   │  自愈机制 (5轮)          │     │    │          │
│  │  │   │  L1语法→L2数据→L3方法   │     │    │          │
│  │  │   │  →L4用户→L5跳过         │     │    │          │
...
│  │  Phase 7: 假设检验+S-R01~R08用户决策       │          │
│  │  Phase 7.5: SAP修正 (如需)                │          │
│  │  Phase 8: 质量检查 (P值+方法+图表)         │          │
│  │  Phase 9: 发表级图表 (SVG+PNG)            │          │
│  │  Phase 10: 审计日志 (不可变)              │          │
│  └──────────────────┬───────────────────────┘          │
│                     ▼                                   │
...
Phase 0: 样本量验证 → 输入:SAP+数据 → 输出:验证报告
  ...
  ▼
Phase 1-2: 变量构造 + 执行前检查 → 输入:SAP规格书 → 输出:分析数据集
  │ 🔴 [ADAPTIVE] 检查不通过时🛑STOP等待用户确认
  ▼
Phase 3-5: 描述统计 + 依从性 + 安全性 → 输出:Table1 + 依从性报告 + 安全性报告
  │ 无确认点，直接进入主分析
  ▼
Phase 6: 代码生成与执行(自愈5轮+[SKIP]) → 输入:SAP方法 → 输出:结果表
  │ 🔴 [ADAPTIVE] ≥2个[SKIP]时🛑STOP讨论是否回退
  ▼
Phase 6.1: 观察性研究高级方法(IPTW/RCS/E-value等) → SAP参数直接使用
  │ 无确认点，参数已在SAP中确认
  ▼
  │ 🔴 [MANDATORY-ME1] 主要分析结果确认🛑STOP（唯一MANDATORY）
  │   展示: 主要效应量+95%CI+p值 + [SKIP]标记数量
  │   决策: 继续 / 修正分析 / 转为exploratory
  ▼
Phase 7-9: 假设检验 + 质检 + 输出 → 输出:10项产物
  │ 🔴 [MANDATORY-S7] 假设检验确认 / 🔴 [MANDATORY-S8] 质检结果确认 / [SLIM] 产物清单确认
```

**设计原则**：
- 预处理阶段(Phase 0-5)：仅在异常时暂停(ADAPTIVE)，正常流程无确认
- 主分析(Phase 6 + 6.1)：仅在有SKIP标记时暂停
- 唯一MANDATORY确认点：Phase 6 完成后的主要分析结果确认
- 后处理(Phase 7-9)：MANDATORY-S7 假设检验 + MANDATORY-S8 质检 + SLIM 产物清单，逐项确认
...
|--------|------|---------|---------|---------|
| Phase 0 样本量 | 自动 | 事件数≥10×变量数 | 事件数<10×变量数 | 标记[SKIP: 事件数不足] |
| Phase 4 变量构造 | 自动 | 全部变量构造成功 | 构造失败 | 记录缺失→标记[PARTIAL] |
| Phase 6 代码执行 | 自动 | 代码执行成功 | 执行失败 | 自愈5轮→[SKIP] |
| Phase 6.2 约束追踪 | 自动 | P/M/D/S约束全通过 | 任何约束违规 | 记录违规→Phase 7处理 |
| Phase 7 假设检验 | MANDATORY-EXEC7 | 所有假设通过 | S-R01~R08违反 | 用户决策→记录选择 |
| Phase 7.5 SAP修正 | MANDATORY | 用户确认修正 | 用户未确认 | 🛑STOP，不继续 |
| Phase 8 质量检查 | 自动 | 全部检查通过 | P值/方法/图表不合规 | 修复→重新检查(3次) |
| Phase 9 图表生成 | 自动 | SVG+PNG生成成功 | 生成失败 | 降级: 表格替代[FIGURE_SKIP] |
| Phase 10 审计日志 | 自动 | 日志写入成功 | 写入失败 | 重试3次→[AUDIT_FAIL] |

> **硬阻断项**: Phase 7(S-R01~R08)和Phase 7.5(SAP修正)为硬阻断，必须用户确认
> **自愈上限**: 最多5轮，每轮记录修复详情，超过5轮标记[SKIP]
> **审计日志**: 必须包含Python版本/包版本/随机种子/执行时间/错误详情

**常见错误处理代码示例**：

**R - 完全分离问题处理 (Firth Logistic)**:
```r
...
**Python - 完全分离问题处理 (Firth Logistic)**:
```python
...
#### Phase 6.1.5: E-value（未测量混杂敏感性分析）
