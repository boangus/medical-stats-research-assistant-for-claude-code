...
Phase 3: 推断分析(自愈5轮+[SKIP]) → 输入:SAP方法 → 输出:结果表
  ...
  ▼
  │ 🔴 [MANDATORY-EXEC7] 假设检验确认（Phase 3 标准步骤）
  ▼
Phase 4-6: 质检 + 输出 + 审计 → 输出:13项产物
  │ 🔴 [MANDATORY-S8] 质检结果确认 / [SLIM] 产物清单确认
```
...
- 后处理(Phase 4-6)：MANDATORY-S8 质检 + SLIM 产物清单，逐项确认
...
| Phase 3 假设检验 | MANDATORY-EXEC7 | 所有假设通过 | S-R01~R08违反 | 用户决策→记录选择 |
...
> 🔴 [MANDATORY-EXEC7] 假设检验完成后，必须展示以下内容给用户确认：
...
| 21 | 自愈机制修改SAP预定义的分析方法 | 违反SAP一致性原则 | 自愈仅修复代码错误，不改变分析方法；需修改方法时走Phase 3 SAP修正流程 |
