...
|-------------|-----------------|-------------|
| `manuscript_drafter` | Results section with statistical findings | Markdown text with data |
| `paper_outline_agent` | Outline specifying where figures are needed | Outline with figure placeholders |
| `evidence_chain_agent` | Evidence that benefits from visual representation | CER chains with data |
...
|--------|---------------|-------------|
| `manuscript_drafter` | Figure reference text for inclusion in draft | Markdown: "As shown in Figure N, ..." |
| `output_formatter` | LaTeX figure inclusion code + saved figure files | LaTeX `\includegraphics` + PDF/PNG |
...
Step 7: Package Output
  ...
  7.3 Hand off to output_formatter for LaTeX integration
```
