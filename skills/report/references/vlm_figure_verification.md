...
After the figure_generator generates a figure, an optional verification loop uses a vision-capable LLM to check the rendered figure against the paper's data and APA 7.0 standards. This catches issues invisible in code review: truncated labels, overlapping text, incorrect data rendering, misleading scales.
...
```
Step 1: figure_generator generates figure code
...
Step 5: If any FAIL:
  ...
  - figure_generator modifies code to fix
...
When VLM verification is run, the Figure Package (from figure_generator) includes:
