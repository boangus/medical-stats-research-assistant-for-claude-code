...
| 诊断试验 | STARD（参考 TRIPOD_AI） | 参考标准、盲法、流程图 |
| 预测模型 | `TRIPOD_AI_checklist.md` 27 项 | 缺失处理、模型验证、校准曲线 |
...
| **statcheck 不一致 (❌/❌❌)** | 定位报告原文，核对统计软件输出，修正报告值或自由度 | 无法判定真因时，在报告中加注"待复核"并提示用户提供原始统计输出 |
