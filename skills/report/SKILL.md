...
| statcheck超时 | >60秒无响应 | 降级为[STATCHECK_SKIPPED]，必须在合规报告中注明并要求作者手动核对统计量 |
...
| 触发条件 | 降级策略 | 标记 | 后续操作 |
|---------|---------|------|---------|
| statcheck API 不可用 | 跳过自动验证，人工复核 | 报告标注 [STATCHECK_SKIPPED] | 在合规报告中注明"statcheck 未运行"，要求作者手动核对统计量 |
