...
> 参考：shared/reporting-guidelines/ — CONSORT/STROBE/STARD/PRISMA 检查清单及流程图模板
...
| 横断面 | **STROBE** | 抽样方法、应答率 |
| 系统综述 | **PRISMA** | 流程图、文献筛选、异质性 |
| 诊断试验 | **STARD** | 金标准、盲法、ROC |
...
| 触发条件 | 处理方式 |
|---------|---------|
| python-docx 未安装 | `pip install python-docx` 后重试 |
| 表格内容为空 | 跳过导出，记录日志 |
| 列数过多 (>10列) | 提示用户拆分表格或调整页面方向为横向 |
...
模板选择规则:
  ...
  3. 若 Python 模板存在但执行失败 → 报错停止，不自动降级到 R
     → 输出: "[ERROR] Python 模板执行失败。请安装依赖后重试"
     → 打印具体错误信息
...
> **🔴 [MANDATORY-M5]** 合规检查结果展示后，**必须等待用户确认**。
> - ✅ 全部通过 → 输出最终报告
> - ⚠️ 有问题 → 修正后重新检查，直到通过
> - ❌ 核心项缺失 → 标记为草稿，禁止提交
...
| 14 | 合规检查不通过仍生成最终报告 | 不合规报告无法通过伦理审查和期刊审稿 | [MANDATORY-M5] 合规检查不通过 → 修正 → 重新检查 → 通过后才输出 |
