...
# 4a. 日期顺序
df["admit_dt"] = pd.to_datetime(df["Date of Admission"])
df["discharge_dt"] = pd.to_datetime(df["Discharge Date"])
date_errors = (df["discharge_dt"] < df["admit_dt"]).sum()
if date_errors > 0:
    issues["critical"].append(f"出院日期早于入院日期: {date_errors}条")
    print(f"  🔴 出院 < 入院: {date_errors}条")
else:
    print(f"  ✅ 所有出院日期 >= 入院日期")

same_day = (df["discharge_dt"] == df["admit_dt"]).sum()
print(f"  ℹ️  同一天出入院: {same_day}条 ({same_day/len(df)*100:.1f}%)")
...
# 4c. 房间号
room_ok = df["Room Number"].between(100, 500).all()
if not room_ok:
    bad = df[~df["Room Number"].between(100, 500)]["Room Number"].unique()
    issues["warning"].append(f"房间号超出范围 100-500: {list(bad)}")
else:
    print(f"  房间号范围: {df['Room Number'].min()}~{df['Room Number'].max()} ✅")

# 4d. 计费金额
neg_billing = (df["Billing Amount"] < 0).sum()
if neg_billing > 0:
    neg_min = df[df["Billing Amount"] < 0]["Billing Amount"].min()
    neg_max = df[df["Billing Amount"] < 0]["Billing Amount"].max()
    issues["warning"].append(f"负值计费金额: {neg_billing}条 (${neg_min:.2f} ~ ${neg_max:.2f})")
    print(f"  🟡 负值计费金额: {neg_billing}条 (${neg_min:.2f} ~ ${neg_max:.2f})")
...
if names_with_issues > 50:
    ...
    print(f"  🟡 姓名大小写不一致（抽样检查）")

# === 5. 范围检查 ===
print(f"\n[5] 范围检查")
print(f"  Age: {df['Age'].min()}~{df['Age'].max()} (mean={df['Age'].mean():.1f})")
print(f"  Billing Amount: ${df['Billing Amount'].min():.2f}~${df['Billing Amount'].max():.2f}")
print(f"  Room Number: {df['Room Number'].min()}~{df['Room Number'].max()}")

# === 分类变量 ===
print(f"\n分类变量分布:")
cat_cols = ["Gender", "Blood Type", "Medical Condition", "Admission Type", "Test Results", "Insurance Provider"]
for col in cat_cols:
