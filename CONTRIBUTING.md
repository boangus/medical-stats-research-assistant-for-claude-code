...
- `version` 必须与 `manifest.json` 中的版本一致
- `min_claude_version` 必须 >= `manifest.json` 中的 `min_claude_code_version`
- 必须包含 IRON RULES 章节
...
1. **创建命令文件**：在 `commands/` 目录下创建 `command-name.md`
2. **注册到 manifest.json**：在 `commands` 对象中添加命令定义
3. **指定 entry_point**：可以是 `skills/*/SKILL.md` 或 `commands/*.md`
4. **添加示例**：在 `examples` 数组中提供使用示例
5. **更新版本号**：将 `manifest.json` 和所有 `SKILL.md` 的版本号递增

```json
"/command-name": {
  "id": "command-id",
  "name": "Command Name",
  "entry_point": "skills/pipeline/SKILL.md",
