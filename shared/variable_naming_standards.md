...
>
> 与本文件的关系：
> - `shared/chart-styles/variable_naming_conventions.md` — 代码层三列命名体系（代码变量名 → 显示名映射）
> - `shared/chart-styles/publication_figure_standards.md` — 图表视觉规范（配色、布局、导出）
...
| 文件 | 用途 | 与本文件关系 |
|------|------|------------|
| `shared/chart-styles/variable_naming_conventions.md` | 代码层三列命名体系 | 互补：该文件定义代码变量名映射，本文件定义学术展示标准 |
| `shared/chart-styles/publication_figure_standards.md` | 图表视觉规范 | 互补：该文件定义配色/布局/导出，本文件定义内容命名/术语格式 |
| `shared/statistics-methods/cheatsheet.md` | 统计方法速查 | 参考：统计术语的使用场景 |
| `shared/reporting-guidelines/` | 报告规范清单 | 参考：各指南对变量命名的要求 |
| `shared/sap/sap_standard.md` | SAP 标准 | 前置：SAP 中的变量定义应与本标准一致 |
| `shared/anti-patterns/medical_stats_anti_patterns.md` | 反模式 | 参考：变量命名相关的常见错误 |
