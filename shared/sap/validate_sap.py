...
class SAPValidator:
    ...
    def validate_user_confirmation(self, passport_path: str, sap_id: str = "sap") -> Dict[str, Any]:
        """
        验证用户是否已确认SAP

        Args:
            passport_path: passport.json文件路径
            sap_id: SAP产物ID
        ...
        if not passport_path.exists():
            return {
                "confirmed": False,
                "error": f"Passport文件不存在: {passport_path}",
                "severity": "P0"
            }
        ...
        try:
            ...
            return {
                "confirmed": False,
                "error": f"读取Passport失败: {str(e)}",
                "severity": "P0"
            }

        # 查找SAP用户确认记录
        confirmation_id = f"{sap_id}_user_confirmed"
        artifacts = passport_data.get("artifacts", [])
        confirmation_artifact = None

        for artifact in artifacts:
            if artifact.get("id") == confirmation_id:
                confirmation_artifact = artifact
                break

        if not confirmation_artifact:
            return {
                "confirmed": False,
                "error": "SAP用户确认记录不存在，必须先确认SAP才能继续",
                "severity": "P0",
                "recommendation": "请先执行Phase 4.5 SAP用户强制确认流程"
            }

        if confirmation_artifact.get("status") not in ("completed", "verified", "consumed"):
            return {
                "confirmed": False,
                "error": f"SAP用户确认状态异常: {confirmation_artifact.get('status')}",
                "severity": "P0",
                "recommendation": "请重新执行SAP用户确认流程"
            }

        confirmation_data = confirmation_artifact.get("confirmation_data", {})
        return {
            "confirmed": True,
            "confirmed_by": confirmation_data.get("confirmed_by", "unknown"),
            "confirmed_at": confirmation_data.get("confirmed_at", "unknown"),
            "confirmation_type": confirmation_data.get("confirmation_type", "unknown"),
            "severity": "INFO"
        }

    def generate_validation_report(self, validation_result: Dict[str, Any],
                                 user_confirmation_result: Dict[str, Any] = None) -> str:
        """
        生成验证报告

        Args:
            validation_result: 验证结果
            user_confirmation_result: 用户确认验证结果（可选）
