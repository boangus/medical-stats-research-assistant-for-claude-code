# MSRA Unified Pipeline — Routing Discipline v1.0.0

> 本文件定义统一项目的路由规则和跨 skill 数据传递规范。
> 版本: v1.0.0 (MSRA 统一流水线)
...
| **exploratory-causal** | `skills/exploratory-causal/SKILL.md` | Worker | 探索性因果分析 (Stage 1.8): 因果发现+混杂识别+假设生成 |
...
| **peer-review** | `skills/peer-review/SKILL.md` | Worker | 医学期刊同行评审 (5 审稿人: 主编+方法学+临床+统计+伦理) |
| **systematic-survey** | `skills/systematic-survey/SKILL.md` | Worker | 医学系统综述检索 (PubMed/Embase/Cochrane + PRISMA + GRADE) |

> 注：pipeline 和 medical-paper 已分别合并到 pipeline（§11）和 report（论文写作模式）。
...
| 探索性分析 | 探索、因果、DAG、混杂、发现、数据驱动、不知道研究什么 | `/msra-explore` |
| 已知研究方向 | "X对Y的影响"、暴露、结局、研究假设 | 直接引导输入 → Stage 2 |
| 报告生成 | 报告、表格、图表、CONSORT、STROBE | `/msra-report` |
| 写论文 | 论文、manuscript、投稿、paper | Stage 5 或 `/msra-full` |
| 文献检索 | 文献、检索、综述、literature | `/msra-write --mode lit-review` 或 `systematic-survey` |
| 论文评审 | 审稿、评审、review | `/msra-reviewer` |
| 系统综述 | 系统综述、PRISMA、systematic | `systematic-survey` (systematic-review mode) |
...
| C | 文献 seed | Phase 1 文献对比 | Stage 5.1 systematic-survey |
