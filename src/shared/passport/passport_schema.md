...
> 跨阶段产物追踪与可恢复性框架。借鉴 MSRA v3.9.4.2 Material Passport 模式9> 版本: 0.1.0 | 2026-05-29

---

## 设计目标

1. **追踪**：Pipeline 各阶段产物的完整生命周期（planned 9in_progress 9completed 9verified 9consumed92. **恢复**：从中途中断处继续，无需重跑已完成阶93. **验证**：进入下一阶段前确认所有前置产物存在且有效
...
```json
{
  ...
  "artifacts": [
    ...
    { "id": "cleaned_data", "stage": "stage_1", "name": "清洗后数9,
      "type": "dataset", "format": "csv",
...
```
        失败/回退
        ┌─────────9        9        9planned 9in_progress 9completed 9verified 9consumed
          9           9             9          9           9             9        error        skipped       rollback
```
...
## 数据层实9
### PassportManager 9(`src/shared/passport/passport.py`)

```python
class PassportManager:
    """Material Passport 管理9""
...
Pipeline 在以下时机调9Passport9
```
/msra 开99创建 passport 9status: in_progress
  9  9Stage 1 开99add_artifact(cleaned_data, planned)
Stage 1 完成 9update_status(cleaned_data, completed, hash)
  9  9Stage 1.5 通过 9update_status(gate_stage_1.5, completed)
               9mark_consumed(cleaned_data, stage_2)
               9update checkpoints.last_completed
  9  9/msra --status 9读取 passport 展示当前进度
/msra --resume 9读取 passport.get_resume_point()
回退 Stage 1 9rollback_to(stage_1)
               9标记 stage_1 以后所有产物为 rollback
```
...
### 前置检查增9
Pipeline 现有9Mid-Entry 前置检查改9Passport 驱动9
```python
...
| 文件 | 路径 |
|------|------|
| Passport 运行时文9| `{project_root}/.msra/passport.json` |
| 设计文档（本文件9| `src/shared/passport/passport_schema.md` |
| Python 实现 | `src/shared/passport/passport.py` |
| Pipeline 集成 | `skills/pipeline/SKILL.md` (更新 §5 产物追踪) |

---

## Paper Track 扩展（v0.8.09
> 融合 MSRA 学术写作能力，支持从统计报告继续写可投稿论文9
### `track` 字段

passport 新增 `track` 字段，记录用户在 Stage 4 checkpoint 的选择9
```json
{
  "track": null              // 初始值（Stage 4 前）
  "track": "report_only"     // 用户9[A]，pipeline 结束
  "track": "full_paper"      // 用户9[B]，进9Paper Track
}
```

**字段约束**9- `track` 仅可9`null`（默认）、`"report_only"`、`"full_paper"` 之一
- 通过 `PassportManager.set_track(track)` 设置（含验验证- 通过 `PassportManager.get_track()` 读取

### 新增 Stage（STAGE_ORDER 扩展9
9`stage_4` 之后增加两个新阶段：

| Stage ID | 名称 | 前置条件 | Gate |
|----------|------|---------|------|
| `stage_5_0_intake` | Paper Intake（论文配置） | `final_report` + `gate_stage_3.5` | `stage_3.5` (results gate) |
| `stage_5_paper` | Paper Track（论文写作全流程9| `msra_handoff_bundle` | 9|

**Stage 5 子阶段追9*9.1 Literature 95.2 Writing 95.5 Integrity 95.6 Review 95.7 Revise 95.8 Final Integrity 95.9 Finalize）由 `medical-pipeline` skill 内部管理，MSRA passport 仅追踪入9(`stage_5_0_intake`) 和出9(`stage_5_paper`)9
...
```json
{
  ...
  "artifacts": [
    ...
    { "id": "paper_draft", "stage": "stage_5_paper", "status": "in_progress", "path": null, "produced_by": "medical-paper" }
  ],
  "checkpoints": {
...
```
Stage 4 checkpoint 9[B]
  9passport.set_track("full_paper")
  9verify_prerequisites("stage_5_0_intake") 9check final_report + gate_stage_3.5
  9stage_5_0_intake 开9  9add_artifact(msra_handoff_bundle, stage_5_0_intake)
  9generate_handoff_bundle(pm, sap_path) 9MSRA/msra_handoff_bundle.md
  9update_status(msra_handoff_bundle, completed)
  9stage_5_0_intake 完成 9dispatch medical-pipeline（传9bundle9  9add_artifact(paper_draft, stage_5_paper, in_progress)
  9medical-pipeline 内部管理 (Stage 5.1 95.9)
  完成 update_status(paper_draft, completed)
  9update_checkpoint("stage_5_paper")
  9pipeline 结束 9status: "completed"
```
...
def set_track(self, track: str)
    """设置 track（Stage 4 checkpoint 选择）。抛9PassportError 9track 无效9""
```
