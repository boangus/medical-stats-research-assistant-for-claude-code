...
> MSRA 项目专用。Stage 4（统计报告）9Stage 5（论文写作）的数据传递契约9> 9`scripts/generate_msra_handoff_bundle.py` 自动填充，亦可手工填写9> 对齐 `resources/shared/handoff_schemas.md` Schema 9 (Material Passport) 9Schema 11 (R&R Traceability Matrix)9>
...
| source | resources/journal-templates/ | 模板源路9|
...
> 配置锁定文件，对9`resources/shared/artifact_reproducibility_pattern.md` 标准结构9> 此块作为 Material Passport (Schema 9) 的可选子字段存在9
```yaml
repro_lock:
