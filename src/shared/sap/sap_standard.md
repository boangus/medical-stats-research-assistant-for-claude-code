...
- 生成脚本：`src/shared/templates/variable_dag_template.py`
```
...
- **硬性上限**：整个 Stage 3 最多 3 次 Amendment（`SAP_AMENDMENT_MAX=3`，已在 `src/shared/passport/passport.py` 中定义为模块级常量）。该上限覆盖所有类别（A/B/C/D）的修正总和，不按类别分别计数。
...
位置: `src/shared/sap/validate_sap.py`
...
位置: `src/shared/sap/sap_to_code.py`
...
位置: `src/shared/sap/track_execution.py`
...
**文件**: `src/shared/sap/templates/sap_template_observational.md`
...
**文件**: `src/shared/sap/templates/sap_template_rct.md`
...
**文件**: `src/shared/sap/templates/sap_template_diagnostic.md`
