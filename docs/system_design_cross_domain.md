...
| C2 | **质量门闸复用** | 现有 3 个 QualityGateChecker（Bio/IMG/RT）均复用 `shared/quality_gates/GateRunner` + `CheckItemResult` + `_build_gate_result()` 模式，cross_domain 需保持一致 | `CrossDomainQualityGateChecker` 完全复用该模式，6 项检查（CD-1.5 三项 + CD-3.5 三项） |
...
| C4 | **Schema 标准化导出** | 需定义 `msra/cross_domain_result/v1` Schema，与 imaging 的 `msra/imaging_features/v1` 格式保持一致 | 新增 `export_v1_schema()` 函数 + `shared/contracts/cross_domain_result_schema.md` 契约文档 |
...
graph TB
    ...
    subgraph "共享层 Shared Layer"
        GATE_RUNNER["shared/quality_gates/<br/>GateRunner + CheckItemResult"]
        CONTRACTS["shared/contracts/<br/>cross_domain_result_schema.md"]
    end
...
| 3 | `shared/contracts/cross_domain_result_schema.md` | Markdown | msra/cross_domain_result/v1 Schema 契约文档 |
...
| **Source Files** | `msra_modules/cross_domain/integration.py` (修改), `msra_modules/cross_domain/__init__.py` (修改), `shared/contracts/cross_domain_result_schema.md` (新建) |
| **Dependencies** | T1 |
| **Priority** | P0 |
...
新建 `shared/contracts/cross_domain_result_schema.md`：Schema 字段定义文档（见 §3.5）。
...
正文结构:
  ...
  11. 质量门闸参考 (引用 shared/quality_gates/)
  12. 输出 Schema 说明
```
