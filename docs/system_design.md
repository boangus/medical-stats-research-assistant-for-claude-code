...
2. **质量门闸设计**: 复用 `shared/quality_gates/` 框架的 `GateRunner + CheckItemResult` 模式，实现 Gate IMG-1（4 项检查）
...
| **GateRunner 框架** | 复用 shared/quality_gates/ 的统一模式，保持 bioinformatics 和 imaging 门闸设计一致 |
...
1. 创建 `quality_gates.py`：实现 `ImagingQualityGateChecker` 类
   ...
   - 复用 `shared/quality_gates/` 的 `CheckItemResult`, `GateResult`, `GateType`, `GateVerdict`
...
```
- 所有质量门闸复用 shared/quality_gates/gate_runner.py 中的:
