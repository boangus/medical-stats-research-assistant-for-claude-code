...
# 6. 打 tag（重要：标记版本以便用户下载特定版本）
#    格式：v<major>.<minor>.<patch>
#    例如：v0.9.1
git tag -a v0.9.1 -m "Release v0.9.1: Medical domain refactoring, protocol adherence framework"
...
- **强烈建议使用语义化版本（Semantic Versioning）**：v`<major>`.`<minor>`.`<patch>`
  - major：破坏性API变更
  - minor：新增功能（向后兼容）
  - patch：bug修复
...
### 删除远程 tag
```bash
git push public --delete v0.9.1
```

### 为特定 commit 打 tag
```bash
git tag -a v0.9.1 <commit-hash> -m "Message"
git push public v0.9.1
```
